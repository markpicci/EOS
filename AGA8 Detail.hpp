#ifndef __AGA8_DETAIL__
#define __AGA8_DETAIL__
...
//     5 - Propane
//     6 - Isobutane         6 - Water
//     7 - n-Butane          7 - Hydrogen sulfide
//     8 - Isopentane        8 - Hydrogen
//     9 - n-Pentane         9 - Carbon monoxide
//    10 - n-Hexane         10 - Oxygen
//    11 - n-Heptane        11 - Isobutane
//    12 - n-Octane         12 - n-Butane
//    13 - n-Nonane         13 - Isopentane
//    14 - n-Decane         14 - n-Pentane
//    15 - Hydrogen         15 - n-Hexane
//    16 - Oxygen           16 - n-Heptane
//    17 - Carbon monoxide  17 - n-Octane
//    18 - Water            18 - n-Nonane
//    19 - Hydrogen sulfide 19 - n-Decane
//    20 - Helium           20 - Helium
//    21 - Argon            21 - Argon
//
...
double RDetail;
enum {NcDetail = 21, MaxFlds = 21, NTerms = 58};
int fn[NTerms+1], gn[NTerms+1], qn[NTerms+1];
double an[NTerms+1], un[NTerms+1];
int bn[NTerms+1], kn[NTerms+1]; // TODO: update VB
double Bsnij2[MaxFlds+1][MaxFlds+1][18+1], Bs[18+1], Csn[NTerms+1];
double Fi[MaxFlds+1], Gi[MaxFlds+1], Qi[MaxFlds+1];
double Ki25[MaxFlds+1], Ei25[MaxFlds+1];
double Kij5[MaxFlds+1][MaxFlds+1], Uij5[MaxFlds+1][MaxFlds+1], Gij5[MaxFlds+1][MaxFlds+1];
double Tun[NTerms+1], Told;
double n0i[MaxFlds+1][7+1], th0i[MaxFlds+1][7+1];
double MMiDetail[MaxFlds+1], K3, xold[MaxFlds+1];
double dPdDsave; //Calculated in the Pressure subroutine, but not included as an argument since it is only used internally in the density algorithm.
double x[NcDetail+1];
...
public:
    ...
    double GetMi(int i){if(i>=0 && i<=NcDetail) return MMiDetail[i+ArrayZero]; return 0.0;}
    double GetXi(int i){if(i>=0 && i<=NcDetail) return x[i+ArrayZero]; return 0.0;}
    std::string GetName(int i){if(i>=0 && i<=NcDetail) return gas[i+ArrayZero];return "";}
    ...
    // Outputs:
    //     Mm - Molar mass (g/mol)
//void MolarMass(double const (&xi)[], int Translate=0)
void MolarMass(std::map<std::string, double>& mix, bool normalize = true)
{
    x[0] = 0.0;
    for(std::size_t i = 1; i <= NcDetail; ++i) x[i] = 0.0;
    for(std::size_t i = 1; i <= NcDetail; ++i)
        x[i] = mix[gas[i]]; //x is ONE based, if xi is 0 based delete one

    double xiTot = 0.0;
    for(std::size_t i = 1; i <= NcDetail; ++i) xiTot += x[i];
    if (normalize && abs(xiTot)>=epsD) for(std::size_t i = 1; i <= NcDetail; ++i) x[i] /= xiTot;

    Mm = 0;
    for(std::size_t i = 1; i <= NcDetail; ++i) Mm += x[i]*MMiDetail[i];
    xTermsDetail();
}
...
void PressureDetail(const double T, const double D, double &P, double &Z)
{
    double ar[3+1][3+1];
...
void Density(const double T, const double P, double &D, int &ierr, std::string &herr)
{
    // Sub DensityDetail(T, P, x, D, ierr, herr)
...
void Alpha0Detail(const double T, const double D, double a0[3]){
